VERSION 5.00
Begin VB.Form FrmCusto 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cadastro de Custos do Produto"
   ClientHeight    =   2520
   ClientLeft      =   150
   ClientTop       =   720
   ClientWidth     =   11910
   ...
   Begin VB.Label Label1 
      BackColor       =   &H80000012&
      Caption         =   " Controle de Custos dos Produtos"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   13.5
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmCusto"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Private LcCarregado, a As Integer
Private LcDescusto As String
...
GlMov = True
If MovImentacao(enAnterior, Custo) Then VinculaDados
GlMov = False
LcRegAtual = False
txt(1).SetFocus
End Sub
...
On Error Resume Next
GlTab = "DecricaoCusto"
GlSq = "Select * from DecricaoCusto where cod='" & txt(0).Text & "'"

If Exclui(Custo) = 1 Then
      VinculaDados
End If
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enPrimeiro, Custo) Then VinculaDados
GlMov = False
LcRegAtual = False
txt(1).SetFocus
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enSeguinte, Custo) Then VinculaDados
GlMov = False
txt(1).SetFocus
LcRegAtual = False
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enultimo, Custo) Then VinculaDados
txt(1).SetFocus
GlMov = False
LcRegAtual = False
End Sub
...
If (LcTipoDados = 1) And (CmdSalvar.Enabled = True) Then
   ...
   SalvaRegistro (Custo)
End If
If (LcTipoDados = 2) And LcAlterado Then SalvaRegistro (Custo)
FechaBanco
GlStringBase = ""
GlordemAnterior = ""
...
On Error Resume Next
If LcTipoDados = 1 Then NovoReg Else Call RegistroAtual(Custo)
