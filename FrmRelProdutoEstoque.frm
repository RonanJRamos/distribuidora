...
'Abertura do relatório de vendas
    ...
    CryRelatorio.ReportFileName = App.Path & "\relProduto.rpt"
    If Iniciado Then LcFormula = "{ALID009.NOME} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{ALID009.NOME} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{ALID009.NOME}='" & UCase(Nome.Text) & "'"
 
    Select Case Ordem.Text
        Case Is = "Nome"
           CryRelatorio.SortFields(0) = "+{ALID009.NOME}"
        Case Is = "Código"
           CryRelatorio.SortFields(0) = "+{ALID009.COD}"
        Case Is = "Preço"
           CryRelatorio.SortFields(0) = "+{ALID009.PTAB}"
        Case Is = "Estoque"
           CryRelatorio.SortFields(0) = "+{ALID009.QuantEstoque}"
        Case Else
           CryRelatorio.SortFields(0) = "+{ALID009.NOME}"
    End Select
...
CryRelatorio.WindowTitle = "Produtos"
