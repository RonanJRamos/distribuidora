...
Begin VB.Form FrmRelPedidoClien 
   ...
   Begin VB.Label Label2 
      BackColor       =   &H00000000&
      Caption         =   "Relatório de Pedido por Cliente"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmRelPedidoClien"
...
AbreBase
LcTam = 0
Set RsEmpresa = Dbbase.OpenRecordset("alid001", dbOpenDynaset, dbSeeChanges, dbOptimistic)
...
'Abertura do relatório de vendas
    ...
    If GlImprimeSemLinha Then
       CryRelatorio.ReportFileName = App.Path & "\RelacaoPedidos.rpt"
    Else
       CryRelatorio.ReportFileName = App.Path & "\RelacaoPedidossl.rpt"
    End If
    If Iniciado Then LcFormula = "{Pedido.CLIENTE} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{Pedido.CLIENTE} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{Pedido.CLIENTE}='" & UCase(Nome.Text) & "'"
    If Len(Nome.Text) > 0 Then
       CryRelatorio.SortFields(0) = "+{Pedido.CLIENTE}"
   End If
...
CryRelatorio.WindowTitle = "Pedidos por Cliente"
...
CryRelatorio.Formulas(5) = "titulo='Pedidos por Cliente'"
