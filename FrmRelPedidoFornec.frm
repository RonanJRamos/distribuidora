...
Begin VB.Form FrmRelPedidoFornec 
   ...
   End
End
Attribute VB_Name = "FrmRelPedidoFornec"
...
AbreBase
LcTam = 0
LcSql = "Select * from alid002 order by razaosic"
Set RsEmpresa = Dbbase.OpenRecordset(LcSql) '"alid002", dbOpenDynaset, dbSeeChanges, dbOptimistic)
...
'Abertura do relatório de vendas
    ...
    CryRelatorio.DataFiles(0) = GLBase
     If GlImprimeSemLinha Then
        CryRelatorio.ReportFileName = App.Path & "\RelacaoPedidosFornecedor.rpt"
     Else
        CryRelatorio.ReportFileName = App.Path & "\RelacaoPedidosFornecedorsl.rpt"
     End If
   
    If Iniciado Then LcFormula = "{Pedido.Fornecedor} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{Pedido.Fornecedor} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{Pedido.Fornecedor}='" & UCase(Nome.Text) & "'"
    If Len(Nome.Text) > 0 Then
       CryRelatorio.SortFields(0) = "+{Pedido.Fornecedor}"
   End If
...
CryRelatorio.WindowTitle = "Pedidos por Fornecedor"
...
CryRelatorio.Formulas(5) = "titulo='Pedidos por Fornecedor'"
