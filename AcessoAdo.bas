...
LcComentario = "- Função 'abreconexao - Criando Variaveis Para abertura do Banco de Dados."

'===> Verifica se o Sistema Esta usando o Access ou MySql
TipoConexao = LeIni("Base de Dados", "tipo de banco", Arqini)
If UCase(TipoConexao) = "ACCESS" Then
    BaseAcess = LeIni("base de dados", "BaseAcess", Arqini)
    'strConnect = "Provider=Microsoft.Jet.OLEDB.3.51;"
    'strConnect = strConnect & "Data Source=" & BaseAcess
    'strConnect = strConnect & ";Mode=" & adModeReadWrite
    
  '  strConnect = "z:\inetpub\dominios\scripts.registrocom.net\web\registro\modelos\" ' ==> Terminar com Barra
    strConnect = "driver={Microsoft Access Driver (*.mdb)};DBQ=" & BaseAcess
   'GLBase = BaseAcess
   For a = Len(BaseAcess) To 1 Step -1
       If Mid(BaseAcess, a, 1) = "\" Then Exit For
   Next
   GlDirRel = Mid(BaseAcess, 1, a) & "rel"
End If
If UCase(TipoConexao) = "MYSQL" Then
    GlIp = LeIni("Base de Dados", "Servidor MySql", Arqini)   ' a localizacao do usuario (localhost)
    GlUser = LeIni("base de dados", "usuariosql", Arqini) ' identificacao
    GlPassword = LeIni("base de dados", "senhasql", Arqini)      ' senha
    GlDataBase = LeIni("base de dados", "nomebancosql", Arqini)
...
End If
If UCase(TipoConexao) = "SQLSERVER" Then
    GlIp = LeIni("Base de Dados", "Servidor MySql", Arqini)   ' a localizacao do usuario (localhost)
    GlUser = LeIni("base de dados", "usuariosql", Arqini) ' identificacao
    GlPassword = LeIni("base de dados", "senhasql", Arqini)      ' senha
    GlDataBase = LeIni("base de dados", "nomebancosql", Arqini)
    strConnect = "driver={SQL Server};server=" & GlIp & ";database=" & GlDataBase & ";uid=" & GlUser & ";pwd=" & GlPassword & ";"
End If
