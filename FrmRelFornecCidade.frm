...
Begin VB.Form FrmRelFornecCidade 
   Caption         =   "Relatório de Fornecedores por Cidade"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6960
   ...
   End
   Begin VB.ComboBox CidadeFornec 
      Height          =   315
      Left            =   120
      TabIndex        =   0
      Top             =   840
   ...
   End
End
Attribute VB_Name = "FrmRelFornecCidade"
...
Private Type DadoCidade
        ...
        Nome As String
End Type
Private MtCidadefornec() As DadoCidade
Private LcTam, a As Long

Private Sub Cidade_Click()
Dim a As Integer
For a = 0 To LcTam
    If MtCidadefornec(a).Nome = CidadeFornec.Text Then
       Nome.Text = MtCidadefornec(a).Codigo
       Exit For
    End If
Next

End Sub
Function carregaCidFornec()
...
Do Until RsEmpresa.EOF
    ReDim Preserve MtCidadefornec(LcTam)
    If Not IsNull(RsEmpresa!Nome) Then
        MtCidadefornec(LcTam).Codigo = RsEmpresa!cod
        MtCidadefornec(LcTam).Nome = RsEmpresa!Nome
        CidadeFornec.AddItem RsEmpresa!Nome
        LcTam = LcTam + 1
    End If
    RsEmpresa.MoveNext
Loop
...
Private Sub CidadeFornec_click()
Dim a As Integer
For a = 0 To LcTam
    If MtCidadefornec(a).Nome = CidadeFornec.Text Then
        Nome.Text = MtCidadefornec(a).Codigo
...
Private Sub CidadeFornec_KeyDown(KeyCode As Integer, Shift As Integer)
...
'Abertura do relatório de vendas
    ...
    If GlImprimeSemLinha Then
       CryRelatorio.ReportFileName = App.Path & "\Frmfornecedores.rpt"
    Else
       CryRelatorio.ReportFileName = App.Path & "\Frmfornecedoressl.rpt"
    End If
    LcFormula = "{ALID002.CIDADE}='" & Nome.Text & "'"
    If Len(Nome.Text) > 0 Then
       CryRelatorio.SortFields(0) = "+{ALID002.CIDADE}"
      End If
...
CryRelatorio.WindowTitle = "Fornecedores por cidade"
...
CryRelatorio.Formulas(5) = "titulo='Fornecedores por cidade'"
