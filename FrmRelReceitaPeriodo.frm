...
Begin VB.Form FrmRelReceitaPeriodo 
   BackColor       =   &H00E6E4D2&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Relatório de Receita Período"
   ClientHeight    =   3120
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5280
   ...
   End
End
Attribute VB_Name = "FrmRelReceitaPeriodo"
...
LcSql = "Select * from alid015 "
If IsDate(Datai.Text) And IsDate(Dataf.Text) Then
    LcWhere = "DTVENC Between '" & Format(Datai.Text, "yyyy-mm-dd") & "' And '" & Format(Dataf.Text, "yyyy-mm-dd") & "'"
End If
If OptQuitados.Value Then
   If Len(LcWhere) > 0 Then LcWhere = LcWhere & " And "
   LcWhere = LcWhere & " VALPAGO>0"
End If
