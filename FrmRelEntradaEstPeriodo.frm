...
Begin VB.Form FrmRelEntradEstPeriodo 
   ...
   End
End
Attribute VB_Name = "FrmRelEntradEstPeriodo"
...
Dbbase.Execute "Delete from entradanf"
'MsgBox Dbbase.RecordsAffected
Set RsNotaMdb = Dbbase.OpenRecordset("Select * from entradanf", dbOpenDynaset, dbSeeChanges, dbOptimistic)
RsNota.Requery
...
Do Until RsNota.EOF
    ...
    For C = 0 To RsNota.Fields.Count - 1
        ...
        If LcNome <> "Sintegra" Then
            If UCase(LcNome) = "VALOR" Then
...
'    CryRelatorio.CopiesToPrinter = Val(Txt1.Text)
  ...
  LcFormula = LcFormula & "{EntradaNf.DATA} >=" & LcChav1 & " And {EntradaNf.DATA} <=" & LcChav2
...
CryRelatorio.WindowTitle = "Entrada de Estoque por Período"
