...
Begin VB.Form FrmRelChequeBanco 
   Caption         =   "Relatório Cheques por Banco"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6960
   ...
   Begin VB.Label Label2 
      BackColor       =   &H00000000&
      Caption         =   "  Relatório de Cheques por Banco"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmRelChequeBanco"
...
'Abertura do relatório de vendas
    ...
    If Not GlImprimeSemLinha Then
       CryRelatorio.ReportFileName = App.Path & "\cheques.rpt"
    Else
       CryRelatorio.ReportFileName = App.Path & "\chequesSl.rpt"
    End If
    If Iniciado Then LcFormula = "{cheques.banco} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{cheques.banco} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{cheques.banco}='" & UCase(Nome.Text) & "'"
    If Len(Nome.Text) > 0 Then
       CryRelatorio.SortFields(0) = "+{cheques.banco}"
      End If
...
CryRelatorio.WindowTitle = "Cheques por banco"
...
CryRelatorio.Formulas(5) = "titulo='Cheques por banco'"
