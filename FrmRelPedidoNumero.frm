...
Begin VB.Form FrmRelPedidoNumero 
   Caption         =   "Relatório Pedido por Número "
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6960
   ...
   Begin VB.Label Label2 
      BackColor       =   &H00000000&
      Caption         =   "  Relatório de Pedido por número "
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmRelPedidoNumero"
...
'Abertura do relatório de vendas
    ...
    Else
       CryRelatorio.ReportFileName = App.Path & "\Pedidosl.rpt"
    End If
    If Iniciado Then LcFormula = "{Pedido.NUMNF} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{Pedido.NUMNF} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{Pedido.NUMNF}='" & UCase(Nome.Text) & "'"
    If Len(Nome.Text) > 0 Then
       CryRelatorio.SortFields(0) = "+{Pedido.NUMNF}"
      End If
...
CryRelatorio.WindowTitle = "Pedido"
