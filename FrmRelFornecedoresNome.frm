...
Begin VB.Form FrmRelFornecedoresNome 
   Caption         =   "Relatório de Fornecedores por Nome"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6960
   ...
   End
End
Attribute VB_Name = "FrmRelFornecedoresNome"
...
'Abertura do relatório de vendas
    ...
    End If
    If Iniciado Then LcFormula = "{ALID002.RAZAOSOC} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{ALID002.RAZAOSOC} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{ALID002.RAZAOSOC}='" & UCase(Nome.Text) & "'"
    If Len(Nome.Text) > 0 Then
       CryRelatorio.SortFields(0) = "+{ALID002.razaosoc}"
      End If
...
CryRelatorio.WindowTitle = "Fornecedores"
...
CryRelatorio.Formulas(5) = "titulo='Fornecedores'"
