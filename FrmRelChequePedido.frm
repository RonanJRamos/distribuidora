...
Begin VB.Form FrmRelChequePedido 
   Caption         =   "Relatório Cheques por Pedido"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6960
   ...
   Begin VB.Label Label2 
      BackColor       =   &H00000000&
      Caption         =   "  Relatório de Cheques por Pedido"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmRelChequePedido"
...
'Abertura do relatório de vendas
    ...
    End If
    If Iniciado Then LcFormula = "{cheques.pedido} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{cheques.pedido} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{cheques.pedido}='" & UCase(Nome.Text) & "'"
    If Len(Nome.Text) > 0 Then
       CryRelatorio.SortFields(0) = "+{cheques.pedido}"
      End If
...
CryRelatorio.WindowTitle = "Cheques por Pedido"
...
CryRelatorio.Formulas(5) = "titulo='Cheques por Pedido'"
