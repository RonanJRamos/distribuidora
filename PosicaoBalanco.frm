...
Begin VB.Form PosicaoBalanco 
   BackColor       =   &H00EAE8DD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Relatório Posição de Balanço"
   ClientHeight    =   3900
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6960
   ...
   End
End
Attribute VB_Name = "PosicaoBalanco"
...
Private Rel       As New CryRelProdutoPosicao
...
LcSql = "Select * from produtos where nome like '" & UCase(Nome.Text) & "%' and Fornecedor like '" & UCase(codigofor.Text) & "%'" & LcWhereDesativado & " order by codigo"
...
Do Until RsNota.EOF
    ...
    LcQMEd = RsNota!QtdMedida
    If IsNumeric(RsNota!QtdMedida) Then
    ...
    Else
        LcQMEd = 1
    End If
    LcSql = " Insert Into relprodutos(codigo,NOME,Custo,CustoTotal,"
    LcSql = LcSql & "UnidMedida,QtdMedida,Preco,QUANTESTOQUE,Desativado)Values("
    LcSql = LcSql & "'" & RsNota!codigo & "','" & RsNota!Nome & "',"
    LcSql = LcSql & "" & Replace(RsNota!Custo, ",", ".") & ","
    LcSql = LcSql & "" & Replace(RsNota!CustoTotal, ",", ".") & ","
    LcSql = LcSql & "'" & LcUnidade & "',"
    LcSql = LcSql & "" & Replace(LcQMEd, ",", ".") & ","
