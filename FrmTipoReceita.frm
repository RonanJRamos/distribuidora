VERSION 5.00
Begin VB.Form FrmTiporeceita 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cadastro de Tipo Receita e Despesas"
   ClientHeight    =   2505
   ClientLeft      =   150
   ClientTop       =   720
   ClientWidth     =   9285
   ...
   Begin VB.Label Label1 
      BackColor       =   &H80000012&
      Caption         =   "Cadastro de Receitas e Despesas"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   13.5
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmTiporeceita"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Private LcCarregado, a As Integer
Private LcDestiporec As String
...
GlMov = True
If MovImentacao(enAnterior, tiporec) Then VinculaDados
GlMov = False
LcRegAtual = False
txt(1).SetFocus
End Sub
...
On Error Resume Next
GlTab = "alid007"
GlSq = "Select * from alid007 where COD='" & txt(0).Text & "'"

If Exclui(tiporec) = 1 Then
      VinculaDados
End If
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enPrimeiro, tiporec) Then VinculaDados
GlMov = False
LcRegAtual = False
txt(1).SetFocus
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enSeguinte, tiporec) Then VinculaDados
GlMov = False
txt(1).SetFocus
LcRegAtual = False
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enultimo, tiporec) Then VinculaDados
txt(1).SetFocus
GlMov = False
LcRegAtual = False
End Sub
...
If (LcTipoDados = 1) And (CmdSalvar.Enabled = True) Then
   GlPergunta = True
   SalvaRegistro (tiporec)
End If
If (LcTipoDados = 2) And LcAlterado Then SalvaRegistro (tiporec)
FechaBanco
GlStringBase = ""
GlordemAnterior = ""
...
On Error Resume Next
If LcTipoDados = 1 Then NovoReg Else Call RegistroAtual(tiporec)
...
If Index = 8 Then
   ...
   If txt(Index).Text <> "R" And txt(Index).Text <> "r" _
   And txt(Index).Text <> "D" And txt(Index).Text <> "d" _
   And txt(Index).Text <> "T" And txt(Index).Text <> "t" Then
      MsgBox "Escolha <R> para RECEITAS  ou <D> para DESPESAS OU <T> para Ambos.", 64, "Aviso"
      txt(Index).Text = ""
      txt(Index).SetFocus
   End If
End If
