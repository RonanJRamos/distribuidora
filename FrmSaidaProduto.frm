...
Begin VB.Form FrmSaidaProduto 
   ...
   Begin VB.ComboBox CFOP 
      Height          =   315
      ItemData        =   "FrmSaidaProduto.frx":0000
      Left            =   8520
      List            =   "FrmSaidaProduto.frx":0022
      TabIndex        =   56
      Text            =   "5.102"
      Top             =   600
      Width           =   1335
   ...
   Begin VB.ComboBox Natureza 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "FrmSaidaProduto.frx":0063
      Left            =   5160
      List            =   "FrmSaidaProduto.frx":0082
      TabIndex        =   2
      Text            =   "VENDAS A VISTA"
      Top             =   600
      Width           =   2775
   ...
   End
End
Attribute VB_Name = "FrmSaidaProduto"
...
Do Until RsItem.EOF
    LcItem = LcItem + 1
    '===> verifica o Estoque
    If Not GlPermitirVendaEstoqueNegativo Then
...
''abreconexao
 Set Rs = AbreRecordset("select * from numeronota", True)
If Rs.EOF Then
'   Rs.AddNew
    LcIncluir = True
Else
    LcNumeroAtual = Rs("NumeroNota")
End If
Set Rs = Nothing
If Natureza.Text <> "SERIE D" Then
   If LcIncluir Then
      LcSq = "insert into numeronota (numeronota) values ('" & Txt(0).Text & "')"
   Else
      If CDbl(LcNumeroAtual) < CDbl(Txt(0).Text) Then
         LcSq = "UPDATE numeronota SET numeronota='" & Txt(0).Text & "'"
      End If
   End If
...
Else
   If LcIncluir Then
      LcSq = "insert into numeronota (numerosd) values ('" & Txt(0).Text & "')"
   Else
      
     LcSq = "UPDATE numeronota SET numerosd='" & Txt(0).Text & "'"
...
LcPesquisa = True
LcSql1 = "Select * from Alid050 where NUMNF='" & LcNumeroOrc & "'"
LcSql2 = "Select * from Alid052 where NUMNF='" & LcNumeroOrc & "' order by item"
LcSql3 = "Select * from ALid001"
LcSql5 = "Select * from ALid200"
LcSql6 = "Select * from produtos"
LcCap = Me.Caption
...
If Natureza.Text <> "SERIE D" Then
  If Len(Txt(0).Text) = 0 Then
     ...
     LcSql = "Select * from numeronota"
     Set RsN = AbreRecordset(LcSql)
     If Not RsN.EOF Then
        If Not IsNull(RsN!numeronota) Then
            LcNumeroNota = Right("000000" & CStr(Val(RsN("numeronota")) + 1), 6)
        Else
             LcSql = "Select * from Alid050 WHERE NATUREZA<>'SD' order by NUMNF"
             'AbreBase
...
'AbreBase
'LcSql = "Se0lect * from Alid201 where nf='" & Txt(0).Text & "'"
...
Dim LcSq As String
LcSql1 = "Select * from Alid050"
'LcSql2 = "Select * from Alid052"
LcSql2 = "Select * from Alid052 where NUMNF = '" & Txt(0).Text & "'"
...
LcComentario = LcComentario & "Alid050"
'Set RsNotaFiscal = Dbbase.OpenRecordset(LcSql1)
LcComentario = LcComentario & "Alid052"
...
If Len(Txt(0).Text) > 0 Then '===> É Alteracao
    LcSq = "Delete from alid050 where numnf='" & Txt(0).Text & "'"
    totalExcl = ExecutaSql(LcSq)
    LcSq = "Delete from alid052 where Numnf='" & Txt(0).Text & "'"
    totalExcl = ExecutaSql(LcSq)
Else
   'RsNotaFiscal.AddNew
End If
...
LcSq = "INSERT INTO alid050 (Numnf,natureza,Dtemis,Status,CFOP,Cliente,Transp,TipoTrans,PlacaTrans,"
...
For a = 0 To UBound(LcMat)
        ...
        If Len(Trim(LcMat(a).CodPro)) > 0 Then
             ...
             LcCom = Replace(LcCom, ",", ".")
             LcSq = "Insert into alid052 (Numnf,item,codprod,qtde,valunit,unimed,qtdum,descricao,icms,cst) values "
             ...
             Else
                ...
                Set RsCheck = AbreRecordset("Select * from alid052 where NumNf='" & Txt(0).Text & "' and CodProd=" & LcMat(a).CodPro, True)
            ...
            End If
             ...
             If Natureza.Text <> "DEVOLUCAO" And Natureza.Text <> "TRANSFERENCIA" Then
                ...
                If LcMat(a).QuanTidadeBaixa > 0 Then
                   If Not Estoque.BaixaEstoque(CDbl(LcMat(a).QuanTidadeBaixa), CDbl(LcMat(a).VUnit), LcMat(a).Und, CDbl(LcMat(a).com)) Then
