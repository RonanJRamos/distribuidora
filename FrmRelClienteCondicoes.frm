...
Begin VB.Form FrmRelClienteCondicoes 
   Caption         =   "Relatório de Clientes por Condições Especiais"
   ClientHeight    =   3090
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6960
   ...
   End
End
Attribute VB_Name = "FrmRelClienteCondicoes"
...
Dim LcEmpresa, LcEndereco, LcFone, LcVer, Lccelular, Lccelular1, Lcemail, LcCap, LcVer1 As String
AbreBase
...
'Abertura do relatório de vendas
    ...
    If GlImprimeSemLinha Then
       CryRelatorio.ReportFileName = App.Path & "\relclientecondicoes.rpt"
    Else
       CryRelatorio.ReportFileName = App.Path & "\relclientecondicoessl.rpt"
    End If
    If Iniciado Then LcFormula = "{ALID001.CondicaoEspecial} like '" & UCase(Nome.Text) & "*'"
    If Qualquer Then LcFormula = "{ALID001.CondicaoEspecial} like '*" & UCase(Nome.Text) & "*'"
    If Igual Then LcFormula = "{ALID001.CondicaoEspecial}='" & UCase(Nome.Text) & "'"
...
CryRelatorio.WindowTitle = "Clientes por Condições Especiais"
...
CryRelatorio.Formulas(5) = "titulo='Clientes por Condições Especiais'"
