VERSION 5.00
Begin VB.Form FrmGalpao 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cadastro de Galpões"
   ClientHeight    =   4500
   ClientLeft      =   150
   ClientTop       =   720
   ClientWidth     =   11910
...
GlMov = True
If MovImentacao(enAnterior, galpao) Then VinculaDados
GlMov = False
LcRegAtual = False
Txt(1).SetFocus
End Sub
...
LcNome = Txt(1).Text
LcCri = "select * from alid013 where ALMOX='" & LcNome & "'"
Set RsEstoque = Dbbase.OpenRecordset(LcCri, dbOpenDynaset, dbSeeChanges, dbOptimistic)
GlTab = "alid012"
GlSq = "Select * from alid012 where codigo='" & Txt(0).Text & "'"

If Exclui(galpao) = 1 Then
...
On Error Resume Next
GlMov = True
If MovImentacao(enPrimeiro, galpao) Then VinculaDados
GlMov = False
LcRegAtual = False
Txt(1).SetFocus
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enSeguinte, galpao) Then VinculaDados
GlMov = False
Txt(1).SetFocus
LcRegAtual = False
End Sub
...
On Error Resume Next
GlMov = True
If MovImentacao(enultimo, galpao) Then VinculaDados
Txt(1).SetFocus
GlMov = False
LcRegAtual = False
End Sub
...
If (LcTipoDados = 1) And (CmdSalvar.Enabled = True) Then
   GlPergunta = True
   SalvaRegistro (galpao)
End If
If (LcTipoDados = 2) And LcAlterado Then SalvaRegistro (galpao)
FechaBanco
GlStringBase = ""
GlordemAnterior = ""
...
On Error Resume Next
If LcTipoDados = 1 Then NovoReg Else Call RegistroAtual(galpao)
