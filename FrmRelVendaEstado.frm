...
Begin VB.Form FrmRelVendaEstado 
   BackColor       =   &H00E6E4D2&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Relatório de Vendas de Produtos Por Substituição "
   ClientHeight    =   3090
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5280
   ...
   End
End
Attribute VB_Name = "FrmRelVendaEstado"
...
LcSql = "Select * from VendaSubstEstado where DTEMIS Between '" & Format(Datai.Text, "yyyy-mm-dd") & "' And '" & Format(Dataf.Text, "yyyy-mm-dd") & "'"

AbreBase
'abreconexao
Set RsNota = AbreRecordsetRel(LcSql, RsNota)
Set RsNotaMdb = Dbbase.OpenRecordset("Select * from VendaSubstEstado", dbOpenDynaset, dbSeeChanges, dbOptimistic)
RsNota.Requery
...
End If
    ...
    'If analitico Then
      ...
      If GlImprimeSemLinha Then
         CryRelatorio.ReportFileName = App.Path & "\VendaSubst.rpt"
      Else
         CryRelatorio.ReportFileName = App.Path & "\VendaSubst.rpt"
      End If
    'Else
  ...
  LcFormula = LcFormula & "{VendaSubstEstado.DTEMIS} >=" & LcChav1 & " And {VendaSubstEstado.DTEMIS} <=" & LcChav2
...
CryRelatorio.WindowTitle = "Vendas Substituição Para o Estado"
